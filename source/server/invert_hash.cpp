...
std::function<bytes (byte_slice)> inline get_hash_fn (crypto::digest<size> (*hash_fn) (byte_slice)) {
    return [hash_fn] (byte_slice bb) -> bytes {
...
net::HTTP::response invert_hash (server &p, 
    ...
    const data::bytes &body) {
        ...
        if (bool (hash_function_param)) {
            ...
            else return error_response (400, INVERT_HASH, problem::invalid_query, "invalid parameter 'function'");
        } else return error_response (400, INVERT_HASH, problem::missing_parameter, "missing required parameter 'function'");

        if (http_method != net::HTTP::method::post && http_method != net::HTTP::method::get) 
            return error_response (405, INVERT_HASH, problem::invalid_method, "use post or get");
        ...
        if (bool (digest_format_param)) {
            ...
            else return error_response (400, INVERT_HASH, problem::invalid_parameter, "invalid parameter 'digest_format'");
        } else if (http_method == net::HTTP::method::get)
            return error_response (400, INVERT_HASH, problem::missing_parameter, "missing required parameter 'digest_format'");
        ...
        if (bool (digest_param)) {
          ...
          if (!bool (digest)) return error_response (400, INVERT_HASH, problem::invalid_parameter, "invalid parameter 'digest'");
        } else if (DigestFormat != digest_format::unset) 
            return error_response (400, INVERT_HASH, problem::missing_parameter, "missing required parameter 'digest'");
        
        data::maybe<const bytes &> data_to_hash;

        if (!bool (content_type)) {
            if (http_method == net::HTTP::method::post) 
                return error_response (400, INVERT_HASH, problem::invalid_query, "data to hash should be provided in body");
        } else if (*content_type != net::HTTP::content::type::application_octet_stream && *content_type != net::HTTP::content::type::text_plain) {
            return error_response (400, INVERT_HASH, problem::invalid_content_type, "either plain text or bytes");
        } else {
            ...
            switch (HashFunction) {
                ...
                default: {
                    return error_response (501, INVERT_HASH, problem::unimplemented, string::write ("hash function ", HashFunction, " is unimplemented"));
                }
            }
            ...
            if (bool (digest) && hash_digest != *digest) 
                return error_response (400, INVERT_HASH, problem::invalid_query, 
                    "provided hash digest does not match calculated");
            
            return error_response (501, INVERT_HASH, problem::unimplemented);
